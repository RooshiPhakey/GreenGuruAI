...
export default function MatrixNamesCanvas({
  ...
  useEffect(() => {
    ...
    function setSize() {
      ...
      speeds = Array(cols).fill(0).map(() => {
        const s = rowsPerSec * (0.9 + Math.random() * 0.3)
        const v = Math.max(rowsPerSec * 0.8, Math.min(rowsPerSec * 1.4, s))
        return v <= 0 ? rowsPerSec : v
      })
  ...
  }, [enabled, fontSize, color, strains, rowsPerSec])
